...
from app.utils.validators import validate_danfe_key
...
def process_pipeline(image: Image.Image) -> str:
    ...
    # 1. Leitura de Código de Barras (Suporta Códigos de Barras gerais e DANFE)
    for angle in ordered_angles:
        ...
        if barcode_result:
            ...
            if len(clean_barcode) >= OCR_MIN_DIGITS:
                logger.info(f"✅ Código de barras identificado ({angle}°): {clean_barcode}")
                return clean_barcode

    # 2. OCR com prioridade para validação matemática
    best_candidate = ""
    ...
    for angle in ordered_angles:
        ...
        # Testa na imagem com tratamento de nitidez e binarização
        for img_to_ocr in [enhanced_pil, cv2_to_pil(rotated_cv)]:
            for psm in [6, 7]:
                ...
                for candidate in candidates:
                    if len(candidate) == DANFE_KEY_LENGTH:
                        if validate_danfe_key(candidate):
                            logger.info(f"🎯 Chave de 44 dígitos 100% Validada (Módulo 11): {candidate}")
                            return candidate
    ...
    if best_candidate:
        logger.info(f"🎯 Resultado extraído: {best_candidate}")
